...
// Application module
angular.module('menendezApp', ['ngValidate']).factory('datosAjax', function ($http) {
  return {
    sendContactoData: function sendContactoData(jsonData) {
  ...
  $scope.contactoSend = function (form) {
    if (form.validate()) {
      jsonData = $scope.prepareDataToSend(); //mostrar mensaje del estado de mail
