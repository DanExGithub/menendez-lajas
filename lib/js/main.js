$(document).ready(function () {
    ...
    function fixNavbarIssue() {
        ...
        function setNewOnClick(originalOnClick, hash) {
            anchorsList[i].onclick = function () {
                ...
                var posOffset =  parseInt(  $(hash).offset().top - $(".sticky-top").height()  );
                console.log("offset:" + posOffset);
                setTimeout(() => {
                    $(document).scrollTop(posOffset);
                }, 200);
                return false;
            };
        }
    ...
    function autoCloseMenu() {
        //Para cerrar automaticamente lista de links del navbar de bootstrap cuando hace click en un enlace, cuando
            ...
            link.addEventListener("click", function () {
                ...
                if (!link.classList.contains("dropdown-toggle")) {
                    if (!isDropdownList) {
                         ...
                         var heightNav = document.getElementById("nav-site").clientHeight;
                         document.getElementById("button-colapse").click();
                         var heightNavUpdate = document.getElementById("nav-site").clientHeight;
                         var offset = window.scrollY + heightNav - heightNavUpdate;
    ...
    function init() {
        //ubicacion,existen 2  mapas, que se muestran segun tamaño de pantalla
        //defino id para que el enlace de "ubicacion funcioe correctamente"
        var isDropdownList = document.getElementById("button-colapse").classList.contains("collapsed");
        if(!isDropdownList){
            document.querySelector("#section_map").classList.remove("container-fluid");
            document.querySelector("#section_map").classList.add("container");
        }
